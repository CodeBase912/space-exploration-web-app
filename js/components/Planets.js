...
export default class Planet {
    constructor(planetRadius, ringsInnerRadius, ringsOuterRadius, ringRotation, name, planetTextureURL, ringTextureURL, position, planetOrbitPosition) {
    ...
    createPlanet() {
        if (this.name == "Sun") {
            const planetGeometry = new THREE.SphereBufferGeometry(this.planetRadius, 64, 64);
    ...
    addRings() {
        ...
        const ringGeometry = new THREE.RingBufferGeometry(this.ringsInnerRadius, this.ringsOuterRadius, 64);
    ...
    addOrbits() {
        const planetOrbitCurve = new THREE.EllipseCurve(
            0,  0,                                      // ax, aY
            this.position, this.position,           // xRadius, yRadius
        ...
        const planetOrbitMaterial = new THREE.LineBasicMaterial( { color : 0xe3e3e3 } );

        // Create the final object to add to the scene
        const planetOrbit = new THREE.Line( planetOrbitGeometry, planetOrbitMaterial );
    ...
    setPlanetOrbitPosition() {
        ...
        const planetLocation = {
            name: this.name,
            x: this.planetBody.position.z * Math.sin(planetRotation) + this.planetBody.position.x * Math.cos(planetRotation),
            y: this.planetBody.position.y,
            z: this.planetBody.position.z * Math.cos(planetRotation) - this.planetBody.position.x * Math.sin(planetRotation),
