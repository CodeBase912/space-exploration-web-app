...
const planets = [sunLocation, mercuryLocation, venusLocation, earthLocation, marsLocation, jupiterLocation, saturnLocation, uranusLocation, neptuneLocation];
...
const innerAsteroidBelt = createAsteroidBelt(innerBeltDimensions.innerRadius, innerBeltDimensions.beltRadius);
scene.add(innerAsteroidBelt);

const outerAsteroidBelt = createAsteroidBelt(outerBeltDimensions.innerRadius, outerBeltDimensions.beltRadius);
...
function removeOrbits() {
    for (let i = 1; i <= 8; i++) {
        scene.children[i].children[scene.children[i].children.length - 1].material.visible = false;
    }
}
...
xhr.onload = function() {
    if (this.status == 200) {
        ...
        diameter.innerHTML = "<span>Diameter:</span> <br>" + theSun.meanRadius * 2 + " km";
        mass.innerHTML = "<span>Mass:</span> <br>" + theSun.mass.massValue + " x 10<sup>" + theSun.mass.massExponent + "</sup>";
        if (theSun.gravity == 0) {
            gravity.innerHTML = "";
        }
        else {
            gravity.innerHTML = "<span>Gravity:</span> <br>" + theSun.gravity + " m.s<sup>-2</sup>";
        }
        ...
        if (theSun.discoveredBy == "") {
            ...
            discoveredBy = "<span>Siscovered by:</span> <br>" + theSun.discoveredBy + " (" + theSun.discoveryDate + ")";
        }
    }
}
